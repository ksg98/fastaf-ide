...
All notable changes to fastestAF will be documented in this file.
...
- **`ai_terminal_*` MCP tools (external agent surface)** — Six tools exposed to external MCP clients (Claude Code, Cursor, …) so a remote agent can observe and drive a fastestAF terminal: `ai_terminal_read_screen`, `ai_terminal_send_input`, `ai_terminal_send_key`, `ai_terminal_wait_for`, `ai_terminal_get_state`, `ai_terminal_get_context`. `send_input`/`send_key` always prompt the user and are rejected while the internal agent loop is active on the target session. Output passes through secret redaction.
...
### Changed
- **GitHub OAuth app** — Moved to fastestAF organization
...
- **PWA Push Notifications** — Web Push from fastestAF directly to mobile PWA clients. VAPID key generation, push subscription management via `/api/push/*` endpoints, service worker with push/notificationclick handlers. Rate limited (1 per session per 30s). iOS standalone detection with guidance
...
### Added
- **Inter-Agent Messaging** — New `messaging` MCP tool for agent-to-agent coordination. Agents register with their `$TUIC_SESSION` identity, discover peers via `list_peers`, and exchange messages via `send`/`inbox`. Dual delivery: real-time push via MCP channel notifications (SSE) when `--dangerously-load-development-channels` is active, plus polling fallback via inbox. Spawned Claude Code agents automatically get the channels flag. fastestAF acts as the messaging hub — no external daemon needed
- **Multi-instance socket coexistence** — Multiple fastestAF instances (e.g. release + dev build) now coexist safely. First instance binds `mcp.sock`, subsequent instances fall back to `mcp-{pid}.sock`. Bridge auto-discovers live sockets with `TUIC_SOCKET` env override. Stale sockets cleaned on startup
...
- **CI Auto-Heal** — When CI checks fail on a branch with auto-heal enabled and an active agent terminal, fastestAF fetches the failure logs via `gh run view --log-failed`, waits for the agent to be idle, and injects the logs with a fix prompt. Up to 3 attempts per cycle. Toggle per-branch in the PR detail popover
...
- **Markdown file association** — `.md`/`.mdx` files registered with fastestAF on macOS. Double-clicking a markdown file in Finder opens it directly in fastestAF
...
- **Display name** — "fastestAF" renamed to "fastestAF" across the codebase
...
### Added
- Initial fastestAF implementation
