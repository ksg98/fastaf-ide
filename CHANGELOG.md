...
All notable changes to FastAF will be documented in this file.
...
#### Upstream unreleased (after v1.7.6, as of 2026-09-03)

#### Upstream [1.7.6] - 2026-09-02
...
- **A tab parked in the wrong repo now says which repo is missing.** An agent
  ...
  repository FastAF does not know about. The tab was filed under whichever
...
- **Updating mdkb no longer leaves FastAF connected to the old daemon.**
  The detached daemon now identifies its version through the existing ping, and
  FastAF compares it with the installed binary before accepting a cached
...
#### Upstream [1.7.0] - 2026-07-31

##### Added

- **Spawned agents now return a durable task handle** — `agent action=spawn` gained `task_id` and `poll_interval_ms`. The session exit path drives the task to `completed` or `failed` whether or not anyone is listening, so an orchestrator that disconnects mid-run no longer loses the outcome. Terminal states are immutable: a cancel that races the agent's exit wins and is never overwritten. Tasks live for the FastAF process only — a restart tears down every PTY, so a recovered `working` task would describe an agent that no longer exists. Every field a pre-task client already read keeps its name and type, so classic MCP clients are unaffected.
...
- **Idle Grok tabs no longer remain active** — Grok's long-lived process leaves the shell OSC 133 busy marker set, while its composer remains visible during responses. FastAF now recognizes Grok's animated bottom status row as `Working` and the stable `❯` composer after that row disappears as `Ready`, so tabs return to idle without requiring optional native hooks.
- **Grok can now invoke MCP tools proxied through FastAF** — Grok rejects qualified tool names containing more than one namespace delimiter, which caused it to silently discard every upstream tool while still showing TUIC's native surface. Sessions identifying as `grok-shell-*` now receive the existing `search_tools`, `get_tool_schema`, and `call_tool` surface automatically; the compatibility mode is isolated to that MCP session, survives TUIC restarts through bridge initialize replay, and does not alter the global `collapse_tools` setting or other clients.
...
### Added
- **Clone from GitHub** — New "Clone from GitHub…" entry in the sidebar's Add-Repository menu (and Command Palette). Paste any repository URL (`https://…`, `git@…` SSH, or `owner/repo` shorthand) — or, when GitHub is connected, browse your repositories (newest-pushed first, filterable) and pick one. Clones show live progress and the repository is added as a project automatically. Private github.com repositories authenticate through your existing GitHub sign-in; the token is passed one-shot and never written to the cloned repo's config.
...
- **`ai_terminal_*` MCP tools (external agent surface)** — Six tools exposed to external MCP clients (Claude Code, Cursor, …) so a remote agent can observe and drive a FastAF terminal: `ai_terminal_read_screen`, `ai_terminal_send_input`, `ai_terminal_send_key`, `ai_terminal_wait_for`, `ai_terminal_get_state`, `ai_terminal_get_context`. `send_input`/`send_key` always prompt the user and are rejected while the internal agent loop is active on the target session. Output passes through secret redaction.
...
### Changed
- **GitHub OAuth app** — Moved to FastAF organization
...
- **PWA Push Notifications** — Web Push from FastAF directly to mobile PWA clients. VAPID key generation, push subscription management via `/api/push/*` endpoints, service worker with push/notificationclick handlers. Rate limited (1 per session per 30s). iOS standalone detection with guidance
...
### Added
- **Inter-Agent Messaging** — New `messaging` MCP tool for agent-to-agent coordination. Agents register with their `$TUIC_SESSION` identity, discover peers via `list_peers`, and exchange messages via `send`/`inbox`. Dual delivery: real-time push via MCP channel notifications (SSE) when `--dangerously-load-development-channels` is active, plus polling fallback via inbox. Spawned Claude Code agents automatically get the channels flag. FastAF acts as the messaging hub — no external daemon needed
- **Multi-instance socket coexistence** — Multiple FastAF instances (e.g. release + dev build) now coexist safely. First instance binds `mcp.sock`, subsequent instances fall back to `mcp-{pid}.sock`. Bridge auto-discovers live sockets with `TUIC_SOCKET` env override. Stale sockets cleaned on startup
...
- **CI Auto-Heal** — When CI checks fail on a branch with auto-heal enabled and an active agent terminal, FastAF fetches the failure logs via `gh run view --log-failed`, waits for the agent to be idle, and injects the logs with a fix prompt. Up to 3 attempts per cycle. Toggle per-branch in the PR detail popover
...
- **MCP bridge socket path** — tuic-bridge was looking for the Unix socket in `tuicommander/` instead of `com.fastaf.ide/`, preventing MCP connections from Claude Code and other agents
...
- **Markdown file association** — `.md`/`.mdx` files registered with FastAF on macOS. Double-clicking a markdown file in Finder opens it directly in FastAF
...
- **Display name** — "FastAF" renamed to "FastAF" across the codebase
