...
pub(crate) struct AppConfig {
    ...
    /// Open files from the file browser in a split pane beside the active
    /// terminal instead of replacing the view (VSCode "open to the side").
    #[serde(default = "default_true")]
    pub(crate) open_files_to_side: bool,
    /// Hide file tabs from the main tab bar while they are docked in a split
    /// pane (each pane has its own tab strip). Terminal tabs always stay.
    #[serde(default = "default_true")]
    pub(crate) hide_docked_file_tabs: bool,
    /// Multiview: toggleable grid showing every live terminal across all
    /// repos and branches at once (Cmd+Alt+M).
    #[serde(default = "default_true")]
    pub(crate) multiview_enabled: bool,
    /// Drag on a mouse-reporting TUI (vim, htop) selects text; a plain click
    /// is still forwarded to the app on release. Off = forward immediately.
    #[serde(default = "default_true")]
    pub(crate) terminal_drag_selects: bool,
    /// Show "Import from Claude Code / Codex / Cursor / superset.sh" in the
    /// Add Repository menu.
    #[serde(default = "default_true")]
    pub(crate) import_tools_enabled: bool,
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            ...
            open_files_to_side: true,
            hide_docked_file_tabs: true,
...
pub(crate) fn load_app_config() -> AppConfig {
    ...
    // That alone is not enough: a second FastAF process (e.g. a `make dev` debug
...
struct RepositoryMutationBatch {
    ...
    // FastAF: workspaces and the sort mode live in the same document.
    #[serde(default)]
    workspaces: Vec<KeyedRepositoryMutation>,
    #[serde(default)]
    workspace_order: Option<RepositoryFieldMutation>,
    #[serde(default)]
    active_workspace_id: Option<RepositoryFieldMutation>,
    #[serde(default)]
    sort_mode: Option<RepositoryFieldMutation>,
}
...
fn apply_active_repository_mutation(
    ...
    apply_scalar_field_mutation(document, "activeRepoPath", "active repository", mutation)
}

/// Compare-and-swap one top-level string-or-null field (`activeRepoPath`,
/// `activeWorkspaceId`, `sortMode`). A missing field reads as null. `label`
/// names the field in the conflict message shown to the user.
fn apply_scalar_field_mutation(
    document: &mut serde_json::Map<String, serde_json::Value>,
    field: &str,
    label: &str,
    mutation: &Option<RepositoryFieldMutation>,
    ...
    if !valid_active_repo_path(&mutation.before) || !valid_active_repo_path(&mutation.after) {
        return Err(RepositorySaveError::Invalid(format!(
            "'{field}' before/after values must be a string or null"
        )));
    }
    let current = document
        .get(field)
    ...
    if current != mutation.before {
        return Err(RepositorySaveError::Conflict(format!(
            "{label} changed in another window; reload before retrying"
        )));
    }
    document.insert(field.to_string(), mutation.after.clone());
    Ok(true)
}
...
fn apply_repository_mutation_batch(
    ...
    changed |= apply_keyed_repository_mutations(document, "workspaces", &batch.workspaces)?;
    changed |= apply_order_mutation(document, "workspaceOrder", &batch.workspace_order)?;
    changed |= apply_scalar_field_mutation(
        document,
        "activeWorkspaceId",
        "active workspace",
        &batch.active_workspace_id,
    )?;
    changed |= apply_scalar_field_mutation(document, "sortMode", "sort mode", &batch.sort_mode)?;
    Ok(changed)
}
...
mod tests {
    ...
    fn app_config_round_trip() {
        ...
        let cfg = AppConfig {
            ...
            open_files_to_side: false,
            hide_docked_file_tabs: false,
        ...
        assert!(!loaded.open_files_to_side);
        assert!(!loaded.hide_docked_file_tabs);
        assert!(!loaded.multiview_enabled);
    ...
    fn app_config_serde_default_for_new_fields() {
        ...
        assert!(loaded.open_files_to_side); // defaults to true
        assert!(loaded.hide_docked_file_tabs); // defaults to true
        assert!(loaded.multiview_enabled); // defaults to true
