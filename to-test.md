...
## Open files beside terminal (2026-07-11)

- [VISUAL] With a terminal focused (no split), click a file in the file browser → the window splits vertically: terminal stays on one side, the file opens in an editor pane beside it, focus moves to the file. (`openFileBesideTerminal` in `utils/filePreview.ts`.)
- [VISUAL] Open a second file the same way → it docks into the SAME editor pane as another tab (no third pane).
- [VISUAL] Open a `.md` file → docks as a rendered markdown pane; an image/PDF → preview pane.
- [VISUAL] Settings → Appearance → "Open Files Beside Terminal" off → clicking a file replaces the view (old behavior). With no terminal active (e.g. an editor tab focused), opening a file also uses the old behavior.
- [VISUAL] While split, docked file tabs sit RIGHT-ALIGNED in the main tab bar (VSCode-style: terminal tabs left, file tabs right) with real filenames; clicking one switches the file shown in the pane. The pane's inner strip only appears for multi-terminal groups. Settings → Appearance → "Align Docked File Tabs" off restores inline tabs + pane strip.
- [VISUAL] Select a second terminal from the tab bar (it overlays the split), then open a file → the second terminal docks into the terminal pane alongside the first; the file goes to the file pane.
- [VISUAL] While split with the file pane focused, press "+" (new terminal) → the terminal opens in the TERMINAL pane (left), never burying the file pane; focus moves to it. (`paneTabAssign.assignTabToActiveGroup`.)
- [VISUAL] Split a project (terminal + files), switch to another project via the command palette repo switcher, switch back → the split layout is RESTORED (was lost because the palette pre-set the active repo, corrupting the pane-layout save key; same fix for the parked-repos popover).

## Multiview: all-terminals grid (2026-07-12)

- [VISUAL] With terminals in 2+ repos, press Cmd+Alt+M (or palette "Toggle Multiview") → every live terminal tiles into one grid; each tile has a header with repo color dot, `repo · branch · name`, and busy/input/idle status. Adding a terminal (Cmd+T) or closing one reflows the grid automatically.
- [HUMAN] Click a tile from ANOTHER repo → active repo/branch switches under the hood and typing lands in that tile (cross-branch focus guard must not snap focus back). Double-click a tile HEADER → exits multiview with that terminal full size.
- [HUMAN] Esc with focus OUTSIDE a tile closes multiview; Esc INSIDE a tile goes to the PTY (agent interrupt still works). Cmd+Alt+M always closes.
- [VISUAL] Enter multiview while a split (terminal + file pane) is active, then exit → the split panes are still LIVE (not frozen canvases; grid-channel handoff via the isVisible resubscribe path both directions).
- [VISUAL] Settings → Appearance → "Terminal Multiview" off → shortcut and palette entry inert, open grid closes immediately; persists as `multiview_enabled` in config.json.
- [VISUAL] Terminals in floating windows are excluded from the grid; a grid with zero live terminals shows "No live terminals".
- [VISUAL] Tile header buttons: "–" removes the tile from the grid (terminal keeps running; reappears next time multiview opens), "✕" closes the terminal; both must NOT trigger the tile's click-focus or the header's double-click-maximize.
- [HUMAN] Drag the boundary between two tiles (cursor turns col/row-resize, accent highlight) → adjacent tracks resize with a ~10% minimum; terminals refit their PTYs live; adding/removing/hiding a terminal resets the track sizes to equal.
- [VISUAL] Side panels (file browser etc.) render normally next to the terminal area — regression for a `props.children` literal-text bug in TerminalArea.
- [VISUAL] With 10+ live terminals, the grid caps at 9 tiles (most recently active; the active tab always included) and a "+N more" pill appears top-right; clicking it opens the Activity Dashboard; hiding a tile rotates the next candidate in.
- [HUMAN] Multiview "+" button (top-right, multiview only) opens a small project picker with a search bar on top; typing filters repos, Enter creates a terminal in the top match (switches to that repo), click picks any row; Esc closes the picker WITHOUT closing multiview.
- [VISUAL] Tab bar has a grid-icon Multiview toggle on the right (next to "+", only when the flag is on); hovering it shows a mini preview of the grid layout (tiles with repo-colored edges + names); the icon highlights while multiview is open.
...
## Nezha-dark default theme + drag-select + import from other tools (2026-07-12)

### Theme
- [VISUAL] Fresh config (or after selecting it): "Nezha Dark" appears in Settings → Appearance → Terminal theme and applies near-black chrome (#1a1b1d), periwinkle accent (#7f9aff), and the GitHub-dark ANSI palette in terminals. Switching to any other theme still works and back.
- [VISUAL] On a brand-new install (no config.json), the app launches with Nezha Dark by default, with no flash of the old vscode-dark palette at startup (global.css :root now matches the default).
- [HUMAN] Existing installs whose themes/ dir predates this build won't have nezha-dark.json on disk — the app must still boot (falls back to vscode-dark, warning logged), and dropping nezha-dark.json into the themes dir hot-reloads it into the picker.

### Drag to select in full-screen apps (terminal_drag_selects, default ON)
- [HUMAN] In vim/htop (mouse reporting on): click-drag → text highlights and auto-copies on release, and the TUI does NOT react to the drag (no visual mode / pane jump). A plain click (no drag) still reaches the TUI — sent as press+release on mouseup.
- [HUMAN] Double-click word-select and triple-click line-select work over a mouse-mode TUI. Shift+drag still selects (unchanged). Scroll wheel still drives the TUI (unchanged).
- [HUMAN] Settings → General → "Drag to select in full-screen apps" OFF → old behavior: mousedown forwards immediately and drag drives the TUI.
- [VISUAL] Drag past the top/bottom edge over a TUI → auto-scroll extends the selection. In multiview, clicking a tile still focuses it with the flag ON.
- [HUMAN] Alt/right/middle clicks over a TUI forward immediately as before (heuristic applies to plain left-button only).

### Import from other tools (import_tools_enabled, default ON)
- [HUMAN] Sidebar → Add Repository menu shows "Import from Claude Code / Codex / Cursor / superset.sh…"; the item disappears when the Settings → General toggle is off.
- [HUMAN] The dialog lists projects discovered from ~/.claude/projects, ~/.codex/sessions, Cursor recents (state.vscdb), and ~/.superset/local.db with agent badges + session counts; already-added repos show a badge and are unchecked by default. Missing tools (e.g. no ~/.superset) contribute nothing and cause no error.
- [HUMAN] Importing with "Import chat history" ON adds the repos to the sidebar; first click on an imported repo's branch restores up to 3 newest Claude/Codex sessions as resumable agent tabs (dead sessions are verified against disk and silently dropped). Restore happens exactly once (savedTerminals is consume-once).
- [HUMAN] Importing with chat history OFF adds repos only; no terminals are auto-spawned during import.
- [HUMAN] Cursor-only folders import as plain repos (no sessions); non-git folders land under the "shell" pseudo-branch.

### Copy file path + scroll sensitivity (2026-07-12, follow-up)
