...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/idle%20memory-~300%20MB-4ade80?style=flat-square" alt="~300 MB idle">
</p>
...
| **Terminal rendering** | Custom canvas grid, no DOM per cell | xterm.js in a Chromium `<webview>` |
...
- Multiview (`Cmd+Alt+M`): grid of every live terminal across every repo and branch, up to 9 tiles + overflow, drag-resizable, click to type in place, double-click to jump full-size
- Split panes: vertical (`Cmd+\`) or horizontal (`Cmd+Alt+\`), up to 4 panes, drag-resize
- Drag to select text even over apps that capture the mouse (vim, htop, agent TUIs) — a plain click still reaches the app
...
The plugin directory is derived from the app identifier (`com.fastaf.ide`). On macOS, copy plugins to:

```
~/Library/Application Support/com.fastaf.ide/plugins/
```

Note: the upstream project's `plugins` git submodule was removed from this repo; plugin bundles must be copied into the directory above manually.
...
<p align="center">Apache 2.0 licensed. Zero telemetry. Runs locally.</p>
