...
interface RustAppConfig {
	...
	open_files_to_side?: boolean;
	hide_docked_file_tabs?: boolean;
...
interface SettingsStoreState {
	...
	openFilesToSide: boolean;
	hideDockedFileTabs: boolean;
	multiviewEnabled: boolean;
	terminalDragSelects: boolean;
...
function createSettingsStore() {
	const [state, setState] = createStore<SettingsStoreState>({
		...
		openFilesToSide: true,
		hideDockedFileTabs: true,
	...
	const actions = {
		...
		async hydrate(): Promise<void> {
			try {
				...
				setState("openFilesToSide", config.open_files_to_side ?? true);
				setState("hideDockedFileTabs", config.hide_docked_file_tabs ?? true);
				setState("multiviewEnabled", config.multiview_enabled ?? true);
		...
		/** Toggle opening file-browser files in a split pane beside the terminal */
		setOpenFilesToSide(enabled: boolean): void {
			setState("openFilesToSide", enabled);
			save();
		},

		/** Toggle hiding docked file tabs from the main tab bar while split */
		setHideDockedFileTabs(enabled: boolean): void {
			setState("hideDockedFileTabs", enabled);
			save();
		},

		/** Toggle the multiview grid feature (all live terminals at once) */
		setMultiviewEnabled(enabled: boolean): void {
			setState("multiviewEnabled", enabled);
			save();
		},

		/** Toggle drag-to-select over mouse-reporting TUIs (click still forwarded) */
		setTerminalDragSelects(enabled: boolean): void {
			setState("terminalDragSelects", enabled);
			save();
		},

		/** Toggle the "Import from other tools" entry in the Add Repository menu */
		setImportToolsEnabled(enabled: boolean): void {
			setState("importToolsEnabled", enabled);
			save();
		},
