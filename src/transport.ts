...
const COMMAND_TABLE: Record<string, CommandTableEntry> = {
	...
	create_file: {
		map: (args) => ({
			method: "POST",
			path: "/fs/create",
			body: { repoPath: args.repoPath, file: args.file },
		}),
	},
	create_directory: {
		map: (args) => ({
...
export const INTENTIONALLY_UNMAPPED: ReadonlySet<string> = new Set<string>([
	...
	// Import wizard — scans the local machine's Claude Code / Codex / Cursor /
	// superset state directories, so the sidebar entry is desktop-gated.
	"discover_importable_projects",
