...
const CanvasTerminal: Component<CanvasTerminalProps> = (props) => {
	...
	onMount(async () => {
		...
		let clickCount = 0;
		let lastClickTime = 0;
		// Arbitrates click-vs-drag on left mousedown while an app has mouse
		// reporting on: a drag becomes a text selection, a stationary release
		// is replayed to the PTY as a click (see mouseGesture.ts).
		const gestureArbiter = createClickDragArbiter<{
			col: number;
			row: number;
			absRow: number | null;
			ev: MouseEvent;
		...
		// Word (double-click) / line (triple-click) selection at pos — shared by
		// the plain-selection path and the drag-select path under mouse reporting.
		const applyMultiClickSelect = (pos: { col: number; row: number }, absRow: number) => {
		...
		bindings.listen(canvasRef, "mousedown", (e: MouseEvent) => {
			...
			// Multi-click counter — shared by the forwarding (drag-select) and
			// plain-selection paths below.
			...
			if (currentFrame && currentFrame.mouseMode > 0 && !e.shiftKey) {
				...
				// Drag-select under mouse reporting: defer the press to the arbiter —
				// a drag becomes a selection, a stationary release is replayed as a
				// click in onMouseUp. Double/triple-click selects immediately.
				if (settingsStore.state.terminalDragSelects && e.button === 0) {
					const absRow = viewportRowToAbs(pos.row);
					if (clickCount >= 2 && absRow !== null) {
						gestureArbiter.reset();
						applyMultiClickSelect(pos, absRow);
						e.preventDefault();
						return;
					}
					gestureArbiter.onDown(e.clientX, e.clientY, { col: pos.col, row: pos.row, absRow, ev: e });
					e.preventDefault();
					return;
				}
		...
		const onMouseMove = (e: MouseEvent) => {
			if (currentFrame && currentFrame.mouseMode > 0 && !e.shiftKey) {
				...
				if (typeof st === "object" && st.kind === "start") {
					// Threshold crossed — commit to selection, anchored at mousedown.
					if (st.down.absRow === null) {
						gestureArbiter.reset();
						return;
					}
					selection.start = { col: st.down.col, row: st.down.absRow };
					selection.end = null;
					selection.selecting = true;
					fullRepaintNeeded = true;
					scheduleRepaint();
				...
				// "selecting" falls through — extend the selection, forward nothing
			}
		...
		const onMouseUp = (e: MouseEvent) => {
			if (currentFrame && currentFrame.mouseMode > 0 && !e.shiftKey) {
				...
				if (up.kind === "click") {
					// Press never became a drag — replay it to the PTY as a
					// press+release pair at the original mousedown position.
					if (currentFrame.sgrMouse) {
						writePtyNoScroll(
							sgrMouseSequence(0, up.down.col, up.down.row, true, up.down.ev) +
								sgrMouseSequence(0, up.down.col, up.down.row, false, up.down.ev),
						);
					}
					return;
				}
				...
				// "endSelection" falls through — finalize the drag selection
			}
			stopSelectionScroll();
		...
		function handleWheel(e: WheelEvent) {
			...
			// Scale wheel input by the user's scroll sensitivity (percent, 20–200).
			// Touch scrolling (direct manipulation) and SGR wheel forwarding above
			// (discrete per-tick events) are deliberately left unscaled.
			const dy = e.deltaY * (settingsStore.state.terminalScrollSensitivity / 100);
			const atBottom =
