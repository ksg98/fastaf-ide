...
export interface TerminalProps {
	...
	/** True for the pane-tree copy rendered by PaneTree — hidden while multiview is open */
	inPaneTree?: boolean;
...
export const Terminal: Component<TerminalProps> = (props) => {
	...
	// Check if this terminal instance is displayed. A terminal in a pane-tree
	// group is only visible when it's the active tab of its group — otherwise
	// every tab in the group runs xterm/ResizeObserver/ScrollTracker/PTY-subscribe
	// concurrently, which multiplies background work by the number of tabs per
	// pane. The predicate itself lives in terminalVisibility.ts.
	const isVisible = () => {
		const split = paneLayoutStore.isSplit();
		const groupId = split ? paneLayoutStore.getGroupForTab(props.id) : null;
		return isTerminalVisible({
			alwaysVisible: !!props.alwaysVisible,
			inPaneTree: !!props.inPaneTree,
			multiviewOpen: multiviewStore.state.isOpen,
			multiviewTile: multiviewStore.isTileVisible(props.id),
			isActiveTab: terminalsStore.state.activeId === props.id,
			detached: terminalsStore.isDetached(props.id),
			split,
			inPaneGroup: !!groupId,
			isGroupActiveTab: !!groupId && paneLayoutStore.state.groups[groupId]?.activeTabId === props.id,
		});
	};
