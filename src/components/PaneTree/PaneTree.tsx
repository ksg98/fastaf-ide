...
const PaneGroupView: Component<{
	...
	// When docked file tabs are aligned into the main tab bar, the pane's own
	// strip is redundant for file-only groups; keep it for multi-terminal groups.
	const showTabBar = () =>
		aliveTabs().length > 1 &&
		(!settingsStore.state.hideDockedFileTabs || aliveTabs().some((t) => t.type === "terminal"));
