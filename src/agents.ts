...
/** Claude Code's bypass-permissions ("YOLO") flag. Appended to Claude resume
 *  commands so restored sessions come back in bypass-permission mode without a
 *  prompt (see AGENTS.claude.resumeCommand / sessionDiscovery.resumeWithId). */
...
export const AGENTS: Record<AgentType, AgentConfig> = {
	claude: {
		...
		resumeCommand: `claude --continue ${SKIP_PERMISSIONS_FLAG}`,
		sessionDiscovery: { resumeWithId: (id) => `claude --resume ${id} ${SKIP_PERMISSIONS_FLAG}` },
